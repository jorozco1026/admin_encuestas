  ...
  /*function conf (){
	...
	function conf__cuadro(toba_ei_cuadro $cuadro)	{
      ...
      if (isset($this->s__datos_filtro)) {
        ...
        $this->s__datos_filtro['tipo_pregunta'] = $text_area;
        $datos_text_area = libreria::get_respuestas_text_area_pregunta ($this->s__datos_filtro);
        if( !$datos_text_field &&  !$datos_text_area ) $datos = null;
        else if ( $datos_text_field &&  $datos_text_area ) 
               $datos = array_merge($datos_text_field, $datos_text_area);
        else if ( $datos_text_field ) $datos = $datos_text_field;
        else $datos = $datos_text_area;
			}
